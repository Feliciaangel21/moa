...
function MeetingReplay({ back }: { back: () => void }) {
  ...
  return <Page><div className="moa-replay-page"><button className="moa-back" onClick={back}><ArrowLeft /> 우리 여행으로 돌아가기</button><div className="moa-replay-head"><div><span className="moa-kicker">MEETING REPLAY</span><h1>우리 대리인들은 어떻게 합의했을까?</h1><p>끝난 회의를 라운드별로 다시 볼 수 있어요.</p></div></div><div className="moa-round-nav">{meetingRounds.map((r,i) => <button className={round===i&&!expanded?'active':''} onClick={() => {setRound(i);setExpanded(false)}} key={r.code}><span>{r.code}</span>{r.name}</button>)}</div><div className="moa-replay-controls"><div>{['전체','내 대리인','다른 대리인','심판','Chief'].map((x) => <button className={filter===x?'active':''} onClick={() => setFilter(x)} key={x}>{x}</button>)}</div><div><button onClick={() => setExpanded(!expanded)}>{expanded?'현재 라운드':'전체 보기'}</button><button onClick={() => setSpeed(speed===1?2:1)}>{speed}x</button><button onClick={() => setPlaying(!playing)}>{playing?<Pause weight="fill"/>:<Play weight="fill"/>}{playing?'일시정지':'재생'}</button></div></div><div className="moa-replay-layout"><section className="moa-message-stream" aria-label="대리인 회의 기록"><AnimatePresence>{visible.length ? visible.map((m,i) => <AgentMessage key={`${m.speaker}-${i}`} message={m} delay={playing ? i*.16/speed : 0} sequence={i} />) : <div className="moa-empty"><Info />이 필터에 맞는 기록은 없어요.</div>}</AnimatePresence></section></div></div></Page>
}
